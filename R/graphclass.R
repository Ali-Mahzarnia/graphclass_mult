...
graphclass.default <- function(X = NULL, Y = NULL,
                       ...
                       Groups = NULL,
                       G_penalty_factors = NULL, folds = 5, lambda_selection = TRUE, 
                               alpha = 0, minlambda = 0.1,lambda_index=0,
                       ...) {
  gl_results = list()
  ...
  if(type=="intersection") {
    ...
    gl = logistic_group_lasso_ridge(X, Y, D,
                                    ...
                                    MAX_ITER = MAX_ITER, 
                                    CONV_CRIT = CONV_CRIT, 
                                    MAX_TIME = MAX_TIME, folds= folds,lambda_selection=lambda_selection, alpha= alpha, 
                                    minlambda=minlambda )
    
    #gl_results$beta = gl$best_beta
    #gl_results$b = gl$best_b
    #if (lambda_selection==TRUE) {gl_results$lambda =gl$lambda}
    
    fit = gl$fit
    fit.cv = gl$cv
    
    if (lambda_selection ==TRUE) {gl_results$lambda_index = best_model(fit.cv)} else { gl_results$lambda_index = lambda_index}
    active_set = features(fit)[[gl_results$lambda_index]]
    active_set = as.numeric(active_set[2:length(active_set)]) 

    result_coef_sparse = coef(fit, gl_results$lambda_index) 
    #print(result_coef_sparse)
    #dim(result_coef_sparse)
    coefs = matrix(NA, length(unique(Y)) , (dim(X)[2] +1) )
    coefs[,1] = result_coef_sparse[,1]
    #print(dim(result_coef_sparse))
    coefs[,active_set] = as.matrix(result_coef_sparse[,2:dim(result_coef_sparse)[2]])
    coefs[,-active_set] = 0
    rownames(coefs) =  rownames(result_coef_sparse)
    gl_results$lambda = unlist(fit.cv$lambda[1])[gl_results$lambda_index]
    gl_results$b =  coefs[,1]
    gl_results$beta = coefs[,2:dim(coefs)[2]]
     
    # Train fitting
    predicted_classes = predict(fit , X)
    Yfit <- predicted_classes$classes[,gl_results$lambda_index]
    gl_results$Yfit = Yfit
    gl_results$train_error <- Err(predicted_classes, classes = Y)[gl_results$lambda_index]
    class(gl_results) <- "graphclass"
    # Test fitting
    if(!is.null(Xtest)) {
      #predict.graphclass(gl_results, Xtest)
      predicted_classes = predict(fit , Xtest)
      Yfit_test <- predicted_classes$classes[,gl_results$lambda_index]
      
      if(!is.null(Ytest)) {
        gl_results$test_error =mean(Ytest!=Yfit_test)
      } 
    }
    gl_results$type = "intersection"
    #print(rowSums(gl_results$beta))
    gl_results$active_nodes = active_nodes(colSums(abs(gl_results$beta)), NODES = NODES)
    gl_results$subgraph_active_edges_rate = active_edges_rate(colSums(abs(gl_results$beta)), NODES = NODES)
    gl_results$nonzeros_percentage = mean(colSums(abs(gl_results$beta))!=0)
    gl_results$reduced_beta= colSums(abs(gl_results$beta))
    gl_results$reduced_b= sum(abs(gl_results$b))
    return(gl_results)
